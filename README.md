...
ProductShot Agent ：把一张商品原图转化为可发布营销素材包的连续工作流：商品分析、创意策划、图片生成、质量评分、平台文案、自然语言修改和导出都沉淀在同一个项目上下文里。
...
- 商品图上传：支持 JPG、PNG、WebP，并将原图作为后续流程的统一素材源。
- 多 Agent 工作流：
  - `ProductAnalysisAgent`：分析商品、人群、卖点、风格和原图问题。
  - `CreativePlannerAgent`：生成多个营销创意方向。
  - `PromptEngineerAgent`：把创意方案转换为图片生成 Prompt。
  - `ImageCriticAgent`：从商品清晰度、风格匹配、商业价值、平台适配等维度评分。
...
- 文字模型 Provider：默认 Mock Provider；可通过后端环境变量切换到 DashScope 兼容接口。
